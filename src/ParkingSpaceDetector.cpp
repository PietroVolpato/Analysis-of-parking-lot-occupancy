...
std::vector<Mat> loadImages (const int sequence) {
    ...
    for (int i = 0; i < fileNames.size(); i++) {
        ...
        if (img.empty()) {
            cerr << "Error loading image: " << fileNames[i] << endl;
            continue;
        }
        imgs.push_back(img);
    }
...
std::vector<LineParams> computeLineParams(const std::vector<Vec4i>& lines) {
    ...
    for (const auto& line : lines) {
        ...
        params.push_back({rho, theta, line});
    }
    return params;
}
...
std::vector<RotatedRect> detectParkingSpaces(const std::vector<Vec4i>& lines) {
    std::vector<LineParams> lineParams = computeLineParams(lines);
...
void drawLines (Mat& imgs, const std::vector<Vec4i>& lines) {
    for (const auto& line : lines) {
        cv::line(imgs, Point(line[0], line[1]), Point(line[2], line[3]), Scalar(0, 0, 255), 2);
    }
}
